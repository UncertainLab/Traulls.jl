# Problem 5.14 from Luksan Vlcek collection
# Chained modified HS49 problem

function lv514(n::Int=100)
    ...
    function c!(cx, x)
        ...
        for k = 1:2N
            l = 3 * div(k-1,2)
            if k % 2 == 1
                cx[k] = x[l+1]^2 + x[l+2] + x[l+3] + 4*x[l+4] - 7
            else
                cx[k] = x[l+3]^2 - 5*x[l+5] - 6
            end
        end
    ...
    for k = 1:p
        l = 3 * div(k-1, 2)
        if mod(k,2) == 1
            append!(jc_nzrows, [k, k, k, k])
            append!(jc_nzcols, [l+1, l+2, l+3, l+4])
        else
            append!(jc_nzrows, [k, k]); append!(jc_nzcols, [l+3, l+5])
        end
    end
    ...
    # Starting point
    start_values = [10.0, 7.0, -3.0]
    x = [start_values[mod(i-1, 3) + 1] for i=1:n]
