# Problem 5.15 from Lksan & Vlcek collection (1999)
# Chained HS50 problem

function lv515(n::Int=100)
    ...
    function c!(cx, x)
        ...
        for k = 1:3N
            l = 4*div(k-1,3)
            if mod(k,3) == 1
                cx[k] = x[l+1]^2 + 2*x[l+2] + 3*x[l+3] - 6
            elseif k % 3 == 2
                cx[k] = x[l+2]^2 + 2*x[l+3] + 3*x[l+4] - 6
            else
                cx[k] = x[l+3]^2 + 2*x[l+4] +3*x[l+5] - 6
            end
        end
        return
    end
