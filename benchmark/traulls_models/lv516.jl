# Problem 5.16 from Lksan & Vlcek collection (1999)
# Chained HS51 problem

function lv516(n::Int=100)
    ...
    function r!(rx, x)
        ...
        for i=1:N
            rx[i] = (x[4i-3] - x[4i-2])^2
            rx[N+i] = x[4i-2] + x[4i-1] - 2
            rx[2N+i] = x[4i] - 1
            rx[3N+i] = x[4i+1] - 1
        end
        return
    ...
    function jr!(J, x)
        ...
        for i = 1:N
            J[i, 4i-3] = 2 * (x[4i-3] - x[4i-2])
            J[i, 4i-2] = -2 * (x[4i-3] - x[4i-2])
            J[N+i, 4i-2] = 1
            J[N+i, 4i-1] = 1
            J[2N+i, 4i] = 1
            J[3N+i, 4i+1] = 1
    ...
    for i=1:N
        ...
        push!(jr_nzrows, 2N+i); push!(jr_nzcols, 4i)
        push!(jr_nzrows, 3N+i); push!(jr_nzcols, 4i+1)
    end
    ...
    for k = 1:p
        l = 4 * div(k-1, 3)
        if mod(k, 3) == 1
            append!(jc_nzrows, [k, k]); append!(jc_nzcols, [l+1, l+2])
        elseif mod(k, 3) == 2
            append!(jc_nzrows, [k, k, k]); append!(jc_nzcols, [l+3, l+4, l+5])
        else
            append!(jc_nzrows, [k, k]); append!(jc_nzcols, [l+2, l+5])
        end
    end
