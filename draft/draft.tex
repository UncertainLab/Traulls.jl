...
\begin{document}
	...
	\tableofcontents
	\fabian{Pas de table des matières dans un article}
	
	\fabian{Il faudrait au moins ajouter Stéphane dans les co-auteurs et peut-être aussi Alain. La table des matières devra être retirée dans la version soumise.}
	
	\section{Introduction}
	
	\mfabian{Il faudrait ajouter une revue de littérature.}
	
	The nonlinear least-squares (NLS) problem consists in minimizing an objective function of the form $\frac{1}{2}\|r(x)\|^2$, where $r\colon \RR^n \to \RR^{n_r}$ is a vector of residuals and $\|\cdot\|$ denotes the $\ell_2$ norm. This structure arises naturally in data fitting, parameter estimation, and scientific computing. A distinctive feature of NLS problems is that the Hessian of the objective can be decomposed into a first-order part $J(x)^TJ(x)$, where $J(x)$ is the Jacobian of $r$, and a second-order part involving the individual Hessians of the residuals weighted by their values. The Gauss--Newton (GN) method~\cite{nocedalwright:2006} exploits this decomposition by retaining only the first-order term, which is effective when the residuals are small at the solution but can lead to poor convergence otherwise. This limitation has motivated a rich literature on structured Hessian approximations. The NL2SOL package~\cite{dennisetal:1981,dennis-etal:1989} complements the GN term with a structured quasi-Newton (SQN) correction updated via a secant equation, while the Levenberg--Marquardt method~\cite{levenberg:1944,marquardt:1963} regularizes the GN matrix by a scalar multiple of the identity. Hybrid strategies that switch between GN and full approximations depending on the residual size have also been explored~\cite{albaalifletcher:1985,fletcherxu:1987}. Further structured approaches include factorized secant updates~\cite{yabetakahashi:1991,zhouchen:2010} and secant equations tailored to the least-squares structure~\cite{biggs:1977,huschens1994}; a comprehensive numerical comparison of these techniques can be found in~\cite{lucksan-etal:2019}.
	
	In many applications, the variables are subject to constraints, and in this paper we consider the constrained NLS problem
	\begin{equation}
		\label{pb:cnls}
		\begin{aligned}
		...
		\end{aligned}
	\end{equation}
	where $r$ and the constraint function $c\colon \RR^n \to \RR^{n_c}$ are assumed to be twice continuously differentiable. The set $\Omega$ consists of linear constraints and is defined as
	\begin{equation}\label{eq:linear_constraints}
		\Omega = \left\{ x \in \RR^n \ | \ Ax=b,\ l \le x \le u\right\},
	\end{equation}
	with $A\in\RR^{m\times n}$, $m < n$, $b\in \RR^m$, and $l,u \in \RR^n$. Without loss of generality, some components of the latter two vectors can be set to $\pm \infty$ for unbounded parameters. The simultaneous presence of nonlinear constraints $c(x)=0$ and a polyhedral feasible set $\Omega$ makes problem~\eqref{pb:cnls} considerably more challenging than the unconstrained case.
	
	Extensions of structured NLS methods to the constrained setting are comparatively less developed. Most existing approaches rely on sequential quadratic programming (SQP) frameworks. In particular, \cite{li-etal:2002} propose an SQP method whose Hessian approximation is based on a secant equation derived from~\cite{biggs:1977}, \cite{tjoabiegler:1991} use a BFGS update of the projected Hessian of a merit function, and~\cite{amiribartels:1989} update the projected Hessian in the null space of the active constraints. For problems with equality constraints only, more recent work includes regularized approaches: \cite{bergou-etal:2021} combine a Levenberg--Marquardt approximation of the Lagrangian Hessian with a trust-region method, and~\cite{orbansiquiera:2020} derive a regularized KKT system by adding primal and dual regularization terms.
	
	An alternative to SQP methods is the augmented Lagrangian (AL) framework, also known as the method of multipliers, independently introduced by~\cite{hestenes:1969} and~\cite{powell:1969}, with further theoretical foundations provided by~\cite{rockafellar:1973}. Rather than solving a sequence of quadratic subproblems as in SQP, AL methods penalize the nonlinear constraints in the objective and solve a sequence of simpler subproblems where only the easy constraints remain. For problem~\eqref{pb:cnls}, this amounts to approximately minimizing, at each outer iteration, the AL function over the polyhedral set $\Omega$, reducing the problem to a linearly constrained one. A major implementation of this philosophy is the LANCELOT solver~\cite{conn-etal:1992}, which handles bound-constrained subproblems using a gradient projection technique~\cite{conn-etal:1988b}. The theoretical underpinnings of this approach, including global convergence results, are developed in~\cite{conn-etal:1988a,conn-etal:1991,conn-etal:1993,conn-etal:1996b} and summarized in~\cite[][Chapter 14]{conn-etal:2000}. More recent AL implementations include the work of~\cite{andreani-etal:2008}, who provide convergence results under weaker constraint qualifications, \cite{gillrobinson:2012}, who propose a primal-dual AL method that handles degenerate constraints, \cite{arreckx-etal:2016}, who implement a matrix-free AL approach with support for inequality constraints via slack variables, and~\cite{curtis-etal:2015}, who develop an adaptive AL algorithm with inexact subproblem solves.
	
	The inner minimization over $\Omega$ that arises at each outer AL iteration is a linearly constrained optimization problem. Gradient projection methods have a rich history for such problems: projected search techniques~\cite{moretoraldo:1991} and the TRON solver~\cite{linmore:1999a} have been particularly effective for bound-constrained problems. When linear equality constraints are present, the computation of projections onto the feasible set can be handled efficiently via normal equations or augmented systems~\cite{gould-etal:2001}. When both bounds and linear equalities are present, as in our case, direct projection onto $\Omega$ is less trivial, but the reduced gradient---the projection of the gradient onto the tangent space of the active constraints---provides a practical criticality measure, as used in solvers such as MINOS~\cite{murtaghsaunders:1978} and LSNNO~\cite{tointtuyttens:1992}\pierre{TODO: Trouver des exemples plus récents}.
	
	\pierre{TODO: Préciser notre positionnement par rapport à la littérature.}
	The present work makes several contributions at the intersection of these lines of research. First, we develop an AL algorithm whose inner minimization phase is specifically designed for polyhedral feasible sets of the form~\eqref{eq:linear_constraints}, combining bound constraints with linear equalities. The inner solver relies on a gradient projection technique where the Cauchy point is computed along the projected gradient path, followed by minor iterations using a projected conjugate gradient method. Efficient projection onto the tangent space of the active constraints is achieved through a normal equations approach with an incrementally updated Cholesky factorization. Second, we approximate the Hessian of the AL function by a structured SR1 update that exploits the sum-of-squares structure of both the objective and the penalty term, following the secant equation philosophy of~\cite{biggs:1977,dennisetal:1981} adapted to the AL setting in the spirit of~\cite{li-etal:2002}. While the latter works employ structured quasi-Newton techniques within SQP frameworks, and the LANCELOT solver~\cite{conn-etal:1992} uses an AL framework for bound constrained problems without exploiting the least-squares structure, our approach combines both ingredients. Third, we establish global convergence of the method by adapting the trust region analysis of~\cite{conn-etal:1988a} to account for the polyhedral constraint structure and by relying on the AL convergence framework of~\cite{conn-etal:1996b}.
	...
	\end{equation}
	Another advantage of AL methods is that they come naturally with the update formula for the multipliers:
	...
	\end{equation}
	Provided that the multipliers associated to the active bounds are of appropriate sign, the quantity $\|\proj{T(x)}{\nabla_x \Phi(x,\lambda,\mu)}\|$ is an appropriate criticality measure used in optimization algorithms for linearly constrained optimization such as MINOS~\cite{murtaghsaunders:1978} and LSNNO~\cite{tointtuyttens:1992}\pierre{TODO: Trouver des exemples plus récents}.
	...
	We have described our algorithm with respect to formulation~\eqref{pb:cnls} with equality constraints but we also accept problems with nonlinear inequality constraints of the form $g(x) \ge 0$. We transform the latter into equality constraints by adding non-negative slack variables, which gives the new constraints
	\begin{equation}
		g(x) - \nu = 0,\quad \nu \ge 0.
	\end{equation}.
	...
	\end{equation}
	To follow the conventional notations of quasi-Newton methods, we will note $y_k:= g_{k+1}-g_k$ and denote the right hand side of~\mfabian{\eqref{eq:structured_sr1_secant_eq}} by $y^A_k$ to insist on its link with the AL formulation.
	
	From~\eqref{eq:structured_sr1_secant_eq}, we can derive the SR1 update formula 
	\begin{equation}\label{eq:structured_sr1_update}
		B_{k+1} = B_k + \dfrac{(y^A_k-B_ks_k)(y^A_k-B_ks_k)^T}{\inner{y^A_k-B_ks_k}{s_k}},
	\end{equation}
	if $\inner{y^A_k-B_ks_k}{s_k} \neq 0$. When the denominator in~\eqref{eq:structured_sr1_update} is zero, we simply set $B_{k+1}=B_k$. To avoid numerical errors, we apply the update when
	\begin{equation}\label{eq:structured_sr1_safeguard}
		\left|\inner{y^A_k-B_ks_k}{s_k}\right| \ge \kappa_{sds}\|s_k\|\|y^A_k-B_ks_k\|,
	\end{equation}
	...
	\begin{figure}[ht]
		...
		\end{tikzpicture}
		\caption{Hierarchy of the three iteration levels in the TRAULLS algorithm. The outer loop updates the Lagrange multipliers and penalty parameter. Each outer iteration triggers a sequence of trust-region inner iterations, and each inner iteration computes its step via a Cauchy point followed by minor iterations using the projected conjugate gradient method.}
	...
	\begin{lemma}\label{lemma:majoration_reduced_gradient_norm}
		...
		where $\kappa_{ubg}$\footnote{Upper Bound on the Gradient norm} is the constant defined by \[\kappa_{ubg} := \max\left(1, \max_{x \in \calX} \|\nabla \varphi(x)\|\right),\] 
	...
	\begin{proof}
		...
		\begin{equation}\label{eq:model_decrease_cauchy_at_tr}
			q_k(s_k) \le -\kappa_{fcd} \frac{h_k^2}{8\kappa_{ubg}} \Delta_k \le -\kappa_{fcd} \frac{h_k^2}{\mfabian{64\kappa_{ubg}^2}} \Delta_k,
		\end{equation} 
	...
	\fabian{The result follow the same line, but is still an adaptation to our context. A sketch of the proof could be useful.}
	...
	\pierre{TODO: Vérifier les numéros de subventions et les formulations exactes.}
	The authors would like to thank Mitacs, the Natural Sciences and Engineering Research Council of Canada (NSERC), the Institute for Data Valorization (IVADO), and Hydro-Québec for their financial support.
	...
	The latter, written $A_{|\calA}$, is the restriction of A to the columns corresponding to the indices in $\calA$. Let $\tilde{L}$ be $(m+p)\times(m+p)$ triangular such that $MM^T=\tilde{L}\tilde{L}^T$. We apply the same block structure to $\tilde{L}$, i.e.
	...
	\begin{equation*}
		...
		\end{aligned}
	\end{equation*}
	Therefore, we can form $\tilde{L}$ by first setting $\tilde{L}_{11}=L$, then solve $\mfabian{p}$ lower triangular systems to compute $G_{21}$ and finally compute the Cholesky factor of $I_p-G_{21}G_{21}^T$ to get $\tilde{L}_{22}$.
	...
	The piece-wise arc $s(t) = \proj{\Omega}{x-\mfabian{t}g}-x$ satisfies the constraints
	\begin{itemize}
		\item $As(t) = 0$
	...
	\[\delta_i^- = \mfabian{\min_{(z_i)_j > 0}} \left\{ \mfabian{(s(t_i)_j - s^{(l)}_j)/(z_i)_j}\right\}, \qquad \delta_i^+ = \mfabian{\min_{(z_i)_j < 0}} \left\{\mfabian{(s(t_i)_j - s^{(u)}_j)/(z_i)_j}\right\},\]
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\Repeat
			\State $\psi_i' \gets \mfabian{-\inner{s^{(i)}}{Hz_i}} -\inner{g}{z_i},\ \mfabian{\psi_i''} \gets \inner{z_i}{Hz_i}$
			...
			\State Set $\Delta t \gets - \mfabian{\psi_i'}/\mfabian{\psi_i''}$
			\State Set$s^C \gets s^{(i)} - \Delta t z_i$, \textbf{found $\gets$ \textbf{true}}
			\Else
			\State Set $\Delta t \gets t_{i+1}-t_i$
			\State Compute the next direction $\mfabian{z_{i+1}}$
