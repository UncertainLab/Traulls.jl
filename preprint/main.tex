...
\begin{document}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\State Set $w^+ \gets w + \gamma p$ where $\gamma$ is the smallest factor such that $w^+_i \in \{l^{(k,j)}_i,u^{(k,j)}_i\}$ for $i \in \calF(x_{k,j})$
			\State \textbf{STOP}
			\EndIf
	...
	\begin{proof}
		...
		First, assume that $\|s_k(t_k^{(5)})\|_\infty \le \Delta_k$. By~\eqref{eq:model_bound_t1} and Lemma~\ref{lemma:bound_model_proj_grad_path}, one has $\psi^\prime(t) < -h_k^2 /8 < 0$ for all $t \in  [0,t_k^{(5)}]$, so $\psi$ is strictly decreasing on this segment. Because the Cauchy point is the first local minimizer of $\psi$ subject to the trust region and the trust region is not violated at $t_k^{(5)}$, we must have $t_k^C \ge t_k^{(5)}$. Therefore, 
	...
	\begin{theorem}[\cite{conn-etal:1988a} Theorem 11]
	...
	\begin{lemma}[\cite{conn-etal:2000}, Lemma 14.4.1]\label{lemma:behavior_multipliers}
