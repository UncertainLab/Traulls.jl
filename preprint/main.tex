...
To avoid any bias in computation time caused by Julia's just-in-time compilation, and for all solvers considered in this section, each instance was solved once to warm up, the result of that first solve being discarded, then solved three times with the minimum of the three timings reported. We present our results with performance profiles of type Dolan--Mor\'e~\cite{dolanmore:2002} produced with the \texttt{BenchmarkProfiles.jl} package~\cite{benchmark-profiles-julia}. When comparing different solvers, it is not relevant to consider instances for which different solutions were found. That is why we remove instances on which all solvers considered for a given comparison converge and such that
\begin{equation}\label{eq:discarding_heuristic}
	\dfrac{f_{\max} - f_{\min}}{1 + \max \left(|f_{\max}|,|f_{\min}|\right)} > 10^{-2},
\end{equation}
where $f_{\max}$ and $f_{\min}$ are respectively the maximum and minimum values of the objective obtained. A similar discarding heuristic is also used in~\cite{wachterbiegler:2006, orbansiquiera:2020}.
...
The profiles on residual evaluations display a clear trend as the residual magnitude grows. The GN approximation performs better on the zero-residual instances but worse on non-zero residual, whereas Hybrid-SR1 is consistent for all residual magnitudes. The gap between even increases as the residuals get larger and larger. The hybrid scheme therefore matches GN when the residuals vanish at the solution and takes a decisive lead as soon as the residuals are significant. This is precisely the behavior the switching rule is designed to produce.
...
For these tests, we compare the three solvers against the computation time, the number of residual evaluations and the number of gradient evaluations. After evaluating criterion~\eqref{eq:discarding_heuristic}, we removed from the profiles the nine following instances: HS61, HS77, HS264, the instances with $n=100$ of problems 5.1, 5.4, 5.13 and 5.14 from~\cite{lucksanvlcek:1999}, and the instances with $n=500$ and $n=1000$ of problem 5.16 from the same collection. The profiles of Figure~\ref{fig:perf_solvers} are thus built on the remaining 73 instances. On these, TRAULLS and IPOPT converge on every instance, while Percival reaches the maximum CPU time on the two instances of problem 5.11 from~\cite{lucksanvlcek:1999} having $n=500$ and $n=1000$. As a side note, we also ran IPOPT with exact second derivatives, and this configuration failed on two instances, reaching the maximum number of iterations on HS27 and terminating with a restoration phase failure on HS70.
