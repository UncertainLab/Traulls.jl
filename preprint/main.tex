...
%% RAPPEL : passer a \notesfalse et retirer les \mfabian/\fabian avant soumission.

\def\boxnote#1#2{\ifnotes\fbox{\footnote{\ }}\ \footnotetext{ From #1: #2}\fi}
\def\fabian#1{\boxnote{Fabian}{\color{red}#1}}
\long\def\mfabian#1{{\color{red} #1}}
\def\hfabian#1{}

\def\pierre#1{\boxnote{Pierre}{\color{blue}#1}}
\long\def\mpierre#1{{\color{blue} #1}}
\def\hpierre#1{}

\title{\mfabian{A structured quasi-Newton augmented Lagrangian algorithm for nonlinear least-squares with linear and nonlinear constraints}} %%%%%%%%%%%%

\author{Pierre Borie \orcidlink{0009-0000-1043-5057}\footremember{udem}{Université de Montréal, Department of Computer Science and Operations Research, Montreal, QC, Canada} \\ \texttt{pierre.borie@umontreal.ca} \and Fabian Bastin~\orcidlink{0000-0003-1323-6787}\footrecall{udem} \\ \texttt{bastin@iro.umontreal.ca} \and Stéphane Dellacherie~\orcidlink{0009-0005-9043-9328}\footremember{hq}{Hydro-Québec, Montreal, QC, Canada}\footremember{uqam}{Université du Québec à Montréal, Montreal, QC, Canada} \\ \texttt{dellacherie.stephane@hydroquebec.com}}
...
\begin{document}
	...
	\begin{abstract} 
		\noindent We present an algorithm for solving nonlinear least-squares problems subject to a mix of nonlinear constraints, linear equalities, and bounds. The nonlinear constraints are handled through an augmented Lagrangian reformulation of the objective, while the linear constraints are kept explicit. Each outer iteration therefore requires the approximate minimization of the augmented Lagrangian over a polyhedron, which we carry out with a gradient projection technique combined with a trust-region step computation. To capture the second-order information specific to the least-squares structure, we approximate the Hessian of the augmented Lagrangian by a Gauss--Newton term corrected by a structured quasi-Newton update, activated by a hybrid switching rule. \mfabian{The correction is built from a secant equation that we derive for the augmented Lagrangian itself, so that the curvature of the residuals and the curvature of the nonlinear constraints, weighted by the shifted multipliers, are approximated together; to the best of our knowledge this is the first structured quasi-Newton approximation designed for the augmented Lagrangian of a least-squares problem. As a by-product of the analysis, we isolate the properties of the projected gradient path over a general polyhedron that make Cauchy point based trust region methods applicable there, a result independent of the least-squares setting.} \mpierre{We establish global convergence of both the inner minimization and the overall method to a first-order critical point, and report numerical experiments. Our method outperforms an augmented Lagrangian solver built on the same outer iteration but without least-squares structure, and is competitive in computation time with a state-of-the-art solver for nonlinear programming.}
	\end{abstract}
	...
	\mfabian{It is worth pointing out what this combination leaves uncovered in the existing literature. Structured quasi-Newton methods for least-squares are, with the exceptions discussed above, developed for the unconstrained case. Augmented Lagrangian solvers such as LANCELOT~\cite{conn-etal:1992} and Percival~\cite{arreckx-etal:2016} handle bound-constrained subproblems but treat the objective as a generic nonlinear function. The structured methods available for constrained least-squares operate within an SQP framework~\cite{amiribartels:1989, tjoabiegler:1991, li-etal:2002} and do not keep bounds and linear equalities explicit in their subproblems. Among the methods surveyed above, we are not aware of one combining the three features at once, namely an approximation exploiting the least-squares structure, a feasible set mixing nonlinear equalities with a general polyhedron whose linear constraints are never penalized nor linearized, and a supporting global convergence analysis. It is this combination that our algorithm addresses.}\fabian{\`A v\'erifier avant soumission~: cette revendication de couverture est forte. Confirmer qu'aucun solveur r\'ecent ne combine structure moindres carr\'es, poly\`edre g\'en\'eral maintenu explicite et contraintes non lin\'eaires.}
	
	\mfabian{The present work brings the structured quasi-Newton philosophy of nonlinear least-squares into the augmented Lagrangian framework. Our contributions are the following.}
	\begin{enumerate}[label=(\roman*), leftmargin=*, itemsep=2pt]
		\item \mfabian{We derive a structured secant equation for the Hessian of the AL, given in~\eqref{eq:structured_secant_eq}, which simultaneously captures the curvature carried by the residuals and the one carried by the nonlinear constraints, the latter being weighted by the shifted multipliers $\bar{\lambda}$ produced by the outer iteration. We show that correcting the Gauss--Newton term of the AL by a structured update satisfying this equation yields an approximation that also satisfies the standard secant equation, so that the structure is exploited without sacrificing the usual quasi-Newton properties. To the best of our knowledge, this is the first structured quasi-Newton approximation designed for the AL of a least-squares problem. We complement it with a hybrid switching rule driven by the relative decrease of the AL itself, rather than of the objective, so that the decision to activate the second-order correction reflects the penalized function that is actually minimized.}
		\item \mfabian{We extend the gradient projection inner minimization of~\cite{conn-etal:1988b}, formulated for bound constraints and underlying the LANCELOT solver, to the general polyhedron~\eqref{eq:linear_constraints} in which bounds and linear equalities coexist. The projection onto $\Omega$ is then no longer available in closed form, and the piecewise structure of the projected gradient path, on which both the Cauchy point computation and the convergence argument rest, has to be re-established. We show that the path remains piecewise linear with nested active sets, and we derive an explicit procedure computing its first local minimizer in that setting (Section~\ref{subsec:inner_iteration} and Appendix~\ref{appendix:cauchy_point_computation}).}
		\item \mfabian{We establish global convergence of the resulting method to a first-order critical point of problem~\eqref{pb:cnls}. The analysis covers a feasible set combining nonlinear equalities with a general polyhedron, for which the constraint qualification has to be restated, and it only requires mild growth conditions on the Hessian model, so that it applies to every variant we implement.}
		\item \mfabian{We provide TRAULLS, an open-source Julia~\cite{bezanson-etal:2017} implementation available at \url{https://github.com/UncertainLab/Traulls.jl} and interfaced with the JuliaSmoothOptimizers ecosystem. All the experiments of Section~\ref{sec:numerical_experiments} can be reproduced from the \texttt{benchmark} folder of that repository.}
	\end{enumerate}
	\mfabian{Together, these ingredients yield a solver specifically designed for NLS problems while handling constraints of general form, including nonlinear equalities and inequalities alongside linear equalities and bounds.}
	...
	\end{equation*}
	with $\kappa_{\mathrm{mlt}} \in (0,1)$. This inequality estimates if there is relative progress that can be made in the tangent subspace spanned by the free variables. 
	...
	\[B^{\mathrm{GN}}_ks_k + \tilde{y}_k = y_k,\]
	so the resulting approximation satisfies the standard secant equation $H_ks_k = y_k$.

	\mfabian{It is worth making precise how the approximation~\eqref{eq:structured_secant_eq} differs from the one of~\cite{li-etal:2002}, which is the closest existing proposal. The two works differ in three respects. First, the algorithmic frameworks are distinct: the authors of~\cite{li-etal:2002} operate within an SQP method, so their approximation targets the Hessian of the Lagrangian and is used to build a quadratic subproblem in which the constraints appear linearized, whereas ours targets the Hessian of the AL and is used within a minimization over a fixed polyhedron. In particular, the curvature contributed by the nonlinear constraints enters our secant equation through the shifted multipliers $\bar{\lambda}_{k+1}$, which are updated by the penalty mechanism at every inner iteration, and not through Lagrange multiplier estimates obtained from the subproblem. Second, the secant equations themselves are not of the same nature. The one of~\cite{li-etal:2002} follows the heuristic of Biggs~\cite{biggs:1977}, in which the second-order term is approximated by a single matrix scaled by the residual values, while~\eqref{eq:structured_secant_eq} is assembled from the individual secant equations~\eqref{eq:components_secant_equations} of the residual and constraint Hessians, and therefore reproduces the exact structure of $S_k$. This is what allows the identity $B^{\mathrm{GN}}_ks_k+\tilde{y}_k=y_k$, which has no counterpart in the Biggs-based construction. Third, the class of problems addressed is larger here, since bounds and linear equalities are kept explicit in the subproblems rather than being penalized or linearized, and the method is supported by a global convergence analysis, whereas~\cite{li-etal:2002} is presented without one.}
	...
	\begin{equation}\label{eq:structured_sr1_safeguard}
		\left|\inner{\tilde{y}_k-B_ks_k}{s_k}\right| \ge \kappa_{\mathrm{sds}}\|s_k\|\|\tilde{y}_k-B_ks_k\|,
	\end{equation}
	for $\kappa_{\mathrm{sds}} \in (0,1)$. Inequality~\eqref{eq:structured_sr1_safeguard} is one of the most common safeguards used in SR1 methods~\cite[Section~6.2]{nocedalwright:2006}.
	...
	\begin{equation}\label{eq:hybrid_update_strategy}
		...
		\end{aligned}\right.
	\end{equation}
	where $\kappa_{\mathrm{hyb}}$ is a parameter in $(0,1)$. In~\cite{tjoabiegler:1991}, this update is also tested with the value of the AL where it plays the role of a merit function. As we will see in Section~\ref{sec:numerical_experiments}, it has an important impact on the performance of the algorithm.
	...
	Algorithm~\ref{algo:traulls} requires $\mu_0 > 0$, $\tau > 1$, positive tolerance constants $\omega,\eta,\kappa_\omega,\kappa_\eta,\beta_\omega,\beta_\eta$ and positive stopping tolerances $\omega_*,\eta_*$, the tolerance sequences being updated following~\cite[Chapter~14]{conn-etal:2000}. The constants driving the inner minimization of Algorithm~\ref{algo:trinner_iteration} are tied to the subproblem criticality tolerance, so that at outer iteration $K$ we set $\kappa_{\mathrm{cg}} = \kappa_{\mathrm{mlt}} = \omega_K$, while the safeguard constant $\kappa_{\mathrm{sds}} \in (0,1)$ of~\eqref{eq:structured_sr1_safeguard} is taken as the square root of the relative machine precision.
	
	At the start of each outer iteration, the initial approximation of the second-order terms of the Hessian is $B_0 = 0$, and the hybrid switching scheme~\eqref{eq:hybrid_update_strategy} is monitored through a constant $\kappa_{\mathrm{hyb}} \in (0,1)$, as in~\cite{tjoabiegler:1991}.
	...
	\mpierre{The analysis below is carried out over the polyhedron~\eqref{eq:linear_constraints}, but because our inner-minimization procedure closely resembles the SBMIN algorithm~\cite{conn-etal:1988b} -- used in the bound-constrained inner-minimization phase of LANCELOT~\cite{conn-etal:1992} -- the architecture of the arguments follows the global convergence proof in~\cite{conn-etal:1988a}. Compared to the bound-constrained case, the projection onto $\Omega$ has no closed form, so the active-set arguments must be re-established and the intermediate results re-derived in the polyhedral setting.} \mfabian{More precisely, the polyhedral structure is confined to the first half of the analysis. Lemmas~\ref{lemma:non_increasing_reduced_gradient_norm} to~\ref{lemma:bound_model_proj_grad_path} and the guaranteed decrease inequality~\eqref{eq:bound_step_decrease} are the results that have to be established anew, because the bound-constrained arguments they replace all rest on the projection being computable componentwise. Once that inequality is available, the polyhedron plays no further role and the standard convergence theory of trust region methods applies unchanged.}
	...
	\begin{equation}\label{eq:step_drecrease_wrt_cauchy}
		q_k(s_k) \le \kappa_{\mathrm{fcd}} \ q_k(s_k^C),
	\end{equation}
	for $\kappa_{\mathrm{fcd}} \in (0,1]$. 
	...
	\mfabian{The first lemma has no counterpart in the bound-constrained setting. There, the projection is computed componentwise, each component of the projected path is an explicit monotone piecewise linear function of $t$, and the monotonicity of the active set follows immediately. Over $\Omega$, the components of the projected step are coupled by the linear equalities, so the nesting of the active sets has to be obtained from the geometry of the projection itself.} We now state a first lemma on how the tangent spaces and the reduced gradients at two different positions compare to each other.
	...
	\mfabian{None of the properties established so far involves the objective function, the quadratic model or the trust region: they only describe how the projected gradient path behaves over a polyhedron. We isolate them, as they apply to any trust region method whose step computation relies on a Cauchy point over a set of the form~\eqref{eq:linear_constraints}.}
	\begin{proposition}\label{prop:projected_path_polyhedron}
		\mfabian{Let $P = \left\{x \in \RR^n \ | \ Ax=b,\ l \le x \le u\right\}$ be a non-empty polyhedron with $A$ of full row rank, let $\bar{x} \in P$ and let $g \in \RR^n$. Define the projected gradient path $s(t) = \proj{P}{\bar{x}-tg}-\bar{x}$ for $t \ge 0$, the tangent space $T(t)$ associated with the bounds active at $s(t)$, and the reduced gradient $z(t) = \proj{T(t)}{g}$. Then the following hold.
		\begin{enumerate}[label=(\roman*), leftmargin=*, itemsep=2pt]
			\item The path is piecewise linear. There are finitely many breakpoints $0=t_0<t_1<\ldots<t_p$ outside of which $s$ is differentiable, with $s^\prime(t) = -z(t)$, and $z$ is constant on each interval $[t_i,t_{i+1})$.
			\item The active sets are nested along the path, and so are the tangent spaces and the reduced gradient norms: for all $0<t\le t^\prime$, $\calA(t) \subseteq \calA(t^\prime)$, $T(t^\prime) \subseteq T(t)$ and $\|z(t^\prime)\| \le \|z(t)\|$.
			\item The path is recovered from its reduced gradients by
			\begin{equation}\label{eq:path_integral_representation}
				s(t) = -\int_0^t z(\tau)\,d\tau,
			\end{equation}
			the integral being understood as the sum of the integrals over the segments delimited by the breakpoints.
		\end{enumerate}}
	\end{proposition}
	\begin{proof}
		\mfabian{Statement (i) follows from~\eqref{eq:projected_gradient_path_recursion} and~\eqref{eq:projected_gradient_path_full_expr}--\eqref{eq:projected_gradient_path_derivative}, the number of breakpoints being finite because, by~\eqref{eq:nested_active_sets}, a bound that becomes active stays active. Statement (ii) is~\eqref{eq:nested_active_sets} together with Lemma~\ref{lemma:non_increasing_reduced_gradient_norm}. Statement (iii) is obtained by integrating $s^\prime$ over each segment $[t_i,t_{i+1}]$ and summing, the one-sided derivatives of $s$ at the breakpoints being well defined and equal to $-z_i$ by~\eqref{eq:projected_gradient_path_derivative}.}
	\end{proof}
	\mfabian{In the bound-constrained case, where the projection is computed componentwise and each component of $s(t)$ is an explicit piecewise linear function of $t$, these properties are immediate and are used without comment. Over a general polyhedron the components are coupled by the linear equalities, and~\eqref{eq:path_integral_representation} is what replaces the componentwise description in every argument that follows. Proposition~\ref{prop:projected_path_polyhedron} and the Cauchy point procedure of Appendix~\ref{appendix:cauchy_point_computation}, which computes the first local minimizer of a quadratic along that path, are independent of the augmented Lagrangian context in which we use them.}
	
	\mfabian{The bound-constrained proof of the result below exploits the separability of the projected path, each component being known in closed form. That separability is lost over the polyhedron, and we replace it by the integral representation of the path through its reduced gradients, which follows from~\eqref{eq:projected_gradient_path_derivative} and is controlled by Lemma~\ref{lemma:non_increasing_reduced_gradient_norm}.} We start by establishing an inequality on the decrease of the objective function after taking step $s_k$. This will involve the norm of the projected gradient,
	...
	\begin{lemma}\label{lemma:majoration_reduced_gradient_norm}
		...
		where \[t_k^{(1)}=\frac{h_k}{2\kappa_{\mathrm{ubg}}},\]
		and $\kappa_{\mathrm{ubg}}$ is the constant defined by $\kappa_{\mathrm{ubg}} := \max\left(1, \max_{x \in \calX} \|\nabla \varphi(x)\|\right)$.
	\end{lemma}
	
	\begin{proof} 
		First note that the constant $\kappa_{\mathrm{ubg}}$ is well defined since $\varphi$ is continuously differentiable on $\calX$, the latter being compact by Assumption~\ref{assumption:inner_iterates_compact}.
		...
		\begin{equation*}
			...
			\end{split}
		\end{equation*}
		Choosing $t=t_k^{(1)}$ and because $\|g_k\|\le \kappa_{\mathrm{ubg}}$ by definition of $\kappa_{\mathrm{ubg}}$, we get
		...
		\begin{equation}\label{eq:t1_leq_t2}
			0 < t_k^{(1)} < t_k^{(2)} \le 1.
		\end{equation}
		\mfabian{By Proposition~\ref{prop:projected_path_polyhedron}(iii) applied with $\bar{x}=x_k$ and $g=g_k$, the path admits the integral representation
		\begin{equation}\label{eq:projected_gradient_path_integral_form}
			s_k(t) = -\int_{0}^{t} z_k(\tau)d\tau.
		\end{equation}}
	...
	The next lemma gives an upper bound on the quadratic model $\psi(t)$ in an interval of interest. \mfabian{Its proof is again conducted along the breakpoints of the polygonal path rather than componentwise, and relies on Lemma~\ref{lemma:non_increasing_reduced_gradient_norm} to control the reduced gradient between two consecutive breakpoints.}
	...
	\begin{lemma}\label{lemma:bound_model_proj_grad_path}
		...
		\begin{equation}\label{eq:lemma_ineq_model_derivative_t3}
			\psi^\prime(t) \le -\alpha_k^2 + t_k^{(3)}\kappa_{\mathrm{ubg}}^2\|H_k\|,
		\end{equation}
		for all $t\in T$.
		...
		\begin{equation}\label{eq:t4_def}
			t_k^{(4)} = \min \left(t_k^{(3)}, \dfrac{\alpha_k^2}{2\kappa_{\mathrm{ubg}}^2(\|H_k\|+1)}\right).
		\end{equation}
	\end{lemma}
	...
	\begin{proof}
		...
		\begin{equation}\label{eq:norm_proj_grad_step_ineq}
			\begin{split}
				...
				& \le t\|g_k\| \le t_k^{(3)}\kappa_{\mathrm{ubg}},
			\end{split}
		\end{equation}
		where we have used the facts that for all breakpoints indices $j$, $\|z_i\| \le \|g_k\| \le \kappa_{\mathrm{ubg}}$ and that the scalar $t$ is taken in $T$. For the remaining terms,
		\begin{equation*}
			\|H_kz_i\| \le \|H_k\| \|z_i\| \le \kappa_{\mathrm{ubg}}\|H_k\|.
		\end{equation*}
		...
		\begin{equation}\label{eq:ineq_derivative_model_second_order}
			\left|\inner{s_k(t)}{H_kz_i}\right| \le t_k^{(3)} \kappa_{\mathrm{ubg}}^2 \|H_k\|.
		\end{equation}
		...
		\begin{equation*}
			\psi^\prime(t) \le -\alpha_k^2 + t_k^{(4)} \kappa_{\mathrm{ubg}}^2 \|H_k\| \le -\dfrac{\alpha_k^2}{2},
		\end{equation*}
	...
	We can now establish a bound on the decrease guaranteed by the step. \mfabian{This is the last result of the section in which the polyhedral structure intervenes, and the point at which our development rejoins the bound-constrained theory.}
	\begin{theorem}
		...
		\begin{equation}\label{eq:bound_step_decrease}
			q_k(s_k) \le -\kappa_{\mathrm{mdc}} h_k^2 \min\left(\dfrac{h_k^2}{b_k},\Delta_k\right),
		\end{equation}
		where 
		\begin{equation*}
			\kappa_{\mathrm{mdc}} = \dfrac{\kappa_{\mathrm{fcd}}}{64\kappa_{\mathrm{ubg}}^2}.
		\end{equation*}
		Furthermore, if iteration $k$ is successful, then
		\begin{equation}\label{eq:bound_successful_step_decrease}
			\varphi(x_k) - \varphi(x_k+s_k) \ge \kappa_{\mathrm{sdc}} h_k^2 \min\left(\dfrac{h_k^2}{b_k},\Delta_k\right),
		\end{equation}
		with $\kappa_{\mathrm{sdc}} = \eta_1 \kappa_{\mathrm{mdc}}$.
	\end{theorem} 
	\begin{proof}
		...
		\end{equation}
		for $t \in [0,t_k^{(5)}]$ with \[t_k^{(5)} := \min\left(t_k^{(1)},\dfrac{h_k^2}{8\kappa_{\mathrm{ubg}}^2 (\|H_k\|+1)}\right) = \dfrac{h_k^2}{8\kappa_{\mathrm{ubg}}^2 (\|H_k\|+1)}.\]
		...
		Then, by~\eqref{eq:step_drecrease_wrt_cauchy}, $q_k(s_k) \le \kappa_{\mathrm{fcd}} q_k(s_k^C)$ and it follows that
		\begin{equation}\label{eq:model_decrease_cauchy_in_tr}
			q_k(s_k) \le -\kappa_{\mathrm{fcd}} \frac{h_k^2}{8} t_k^{(5)}.
		\end{equation}
		Now, assume that $\|s_k(t_k^{(5)})\|_\infty > \Delta_k$. The latter implies that $\|s_k(t_k^C)\|_\infty = \Delta_k$ and from
		\begin{equation*}
			\left\|s_k\left(\frac{\Delta_k}{\kappa_{\mathrm{ubg}}}\right)\right\|_\infty \le 	\left\|s_k\left(\frac{\Delta_k}{\kappa_{\mathrm{ubg}}}\right)\right\| \le \frac{\Delta_k}{\kappa_{\mathrm{ubg}}} \|g_k\| \le \Delta_k,
		\end{equation*}
		we can deduce that 
		\begin{equation*}
			t_k^C \ge \frac{\Delta_k}{\kappa_{\mathrm{ubg}}}.
		\end{equation*}
		...
		\begin{equation}\label{eq:model_decrease_cauchy_at_tr}
			q_k(s_k) \le -\kappa_{\mathrm{fcd}} \frac{h_k^2}{8\kappa_{\mathrm{ubg}}} \Delta_k \le -\kappa_{\mathrm{fcd}} \frac{h_k^2}{64\kappa_{\mathrm{ubg}}^2} \Delta_k,
		\end{equation} 
		because $\kappa_{\mathrm{ubg}} \ge 1.$
	...
	\mpierre{We now establish that every limit point of the outer iteration sequence is a first-order critical point of~\eqref{pb:cnls}, for a feasible set combining nonlinear equalities with a general polyhedron. The argument follows the structure of the global convergence proof outlined in~\cite{conn-etal:1996b}, where the linear constraints are inequalities only; the constraint qualification is thus restated to account for the linear equalities present alongside the bounds.}
	...
	We can now proceed with the global convergence proof, starting with a first lemma giving a bound on the reduced gradient norm. \mfabian{This bound is specific to our setting. When $\Omega$ reduces to a box, the comparison between the criticality measure used by the algorithm and the projected gradient on the tangent space at the limit point is obtained componentwise. Here we derive it from the characterization of the projection by the normal cone $\calN_\Omega(v_K)$, whose expression~\eqref{eq:normal_cone} identifies it with the row space of the matrix~\eqref{eq:matrix_tangent_space} gathering the active linear constraints.}
	...
	\mfabian{We now state the main convergence results. Their statement parallels~\cite[Lemma 4.3]{conn-etal:1996b}, established for a feasible set whose linear constraints are inequalities only, and two elements differ here. First, the tangent space $T(x_*)$ is the null space of the matrix $\tilde A_*$ gathering the linear equalities together with the bounds active at $x_*$, and not of the active inequalities alone; the bounds below are consequently expressed in the orthonormal basis $N_*$ of that space. Second, the constraint qualification is Assumption~\ref{assumption:cq_null_space_jacobian}, which imposes the rank condition on $C(x_*)N_*$ for this larger set of active constraints, and it is under that form that the multiplier estimate $\lambda(x)$ is well defined.}\fabian{\`A v\'erifier avant soumission~: l'affirmation selon laquelle le Lemme 4.3 de Conn et al. (1996b) ne traite que des in\'egalit\'es lin\'eaires, et que la diff\'erence se r\'eduit bien aux deux points \'enonc\'es ici.}
	...
	\mpierre{Our first set of experiments compares the behavior and performance of our algorithm depending on the Hessian approximation used during the inner minimization. We compare the GN approximation~\eqref{eq:hessian_gn_approx} against structured approximations of the form~\eqref{eq:hessian_full_approx}. Based on the structured secant equation~\eqref{eq:structured_secant_eq}, we implemented the SR1 update we discussed and the BFGS update that one can derive similarly. Both structured approximations were used in their plain and hybrid variants with the switching rule~\eqref{eq:hybrid_update_strategy}.}
	
	\subsubsection{Profiles on the whole test set}
	
	\mpierre{We first compare these five strategies with respect to three metrics: computation time, number of outer iterations and number of inner iterations. The last metric coincides with the number of residual evaluations, since the trust-region mechanism evaluates the residuals exactly once per inner iteration. Results are reported in Figure~\ref{fig:compare_hessians}.}
	
	\mpierre{Two variants fail to reach a first-order critical point. The plain SR1 variant reaches the maximum number of outer iterations on problem HS373 and returns a point that is only feasible. The plain BFGS variant does the same on the instance of problem 5.1 from~\cite{lucksanvlcek:1999} with $n=500$, although it attains there the same value of the objective as the other variants. Problem HS23 and the instances of problems 5.13 and 5.14 from~\cite{lucksanvlcek:1999} with $n=100$ were removed after evaluating the relative-gap criterion~\eqref{eq:discarding_heuristic}. The profiles are therefore built on the remaining 79 instances, for which all five variants agree on the solution.}
	...
	\mpierre{Computation time, however, favors the GN approximation in every stratum, even though the gap narrows as the residuals grow. The savings in residual evaluations achieved by the hybrid scheme therefore do not compensate for the additional cost of forming and using a dense second-order correction in the subsequent linear-algebra computations. This is largely due to how the test set is built. For all the problems considered, residuals are available by direct computation and their evaluation is consequently very cheap. In many applications, and in particular when the residuals come from a simulation code or from the solution of a system of ordinary differential equations, their evaluation is the dominant computational cost. We argue that, in such settings, the profiles of Figure~\ref{fig:stratified_reseval} are the relevant ones, and they show that the structured correction with hybrid switching does reduce the amount of work that actually matters.}
	
	\mfabian{\subsubsection{How expensive must a residual be?}

	The preceding observation, that the savings in residual evaluations do not translate into savings in time on this test set, calls for a quantitative answer rather than an argument. We therefore ask at which evaluation cost the structured correction starts to pay off. Assume that one residual evaluation costs $c$ more than it does here, every other cost being unchanged. The total time of a solver becomes
	\begin{equation}\label{eq:residual_cost_model}
		T(c) = T + c\, n_r^{\mathrm{eval}},
	\end{equation}
	where $T$ is the time we measured and $n_r^{\mathrm{eval}}$ the number of residual evaluations it performed. Comparing the GN approximation with Hybrid-SR1 on a given instance, the two are equally fast at the break-even cost
	...
	Before applying it, we record in Table~\ref{tab:evaluation_ratios} how the evaluation counts of the two variants compare, stratum by stratum. The pairwise counts are as informative as the geometric means. On the zero-residual instances the two variants perform exactly the same number of evaluations on $17$ of the $18$ instances, which confirms that the switching rule is inert when the correction is not needed and that it costs nothing there. The saving then grows with the residual magnitude, reaching $25\%$ on the medium-residual instances and $31\%$ on the large-residual ones. The Jacobian and gradient counts follow the same trend, which matters here, since an application whose residuals are expensive to evaluate usually has expensive derivatives as well.
	...
	\begin{tabular}{lcccccc}
		...
		small  & 17 &  4 &  7 & 6 & 0.95 & 0.96 \\
		medium & 23 & 14 &  4 & 5 & 0.76 & 0.79 \\
		large  & 24 & 18 &  0 & 6 & 0.67 & 0.70 \\
		\hline
	\end{tabular}
	\caption{Evaluation counts of Hybrid-SR1 relative to the GN approximation, by residual magnitude at the solution. The three middle columns count the instances on which Hybrid-SR1 performs fewer, exactly as many, or more residual evaluations than GN. The last two columns report the geometric mean of the ratio of the Jacobian and gradient evaluation counts, a value below one favoring Hybrid-SR1.}
	\label{tab:evaluation_ratios}
	\end{table}

	We now restrict the analysis to the $47$ instances whose residuals do not vanish at the solution, since these are the ones on which the correction is meant to act. In the test set itself, one residual evaluation takes $0.093$ milliseconds in median. The median break-even cost~\eqref{eq:breakeven_cost} is $0.286$ milliseconds, with a first quartile of $0.003$ and a third quartile of $1.359$. In other words, it is enough that a residual evaluation cost about four times what it costs here, that is a few tenths of a millisecond in total, for Hybrid-SR1 to become the faster variant on the median instance. An evaluation that requires a simulation run or the integration of a system of ordinary differential equations is two to three orders of magnitude above that threshold.

	Figure~\ref{fig:breakeven} shows the proportion of these instances on which Hybrid-SR1 is faster, as a function of the extra cost $c$. It starts at $43\%$ with the times we measured, reaches $51\%$ at $c=0.1$ millisecond and $57\%$ at $c=1$ millisecond. The curve then saturates slightly below $70\%$, and this is the honest limit of the argument. The remaining instances are those on which the hybrid variant does not perform fewer residual evaluations, and no evaluation cost can make it faster on them. The conclusion we draw is therefore a conditional one, namely that the structured correction is the better choice on the majority of the problems with nonzero residuals as soon as the residuals are genuinely expensive, and that the threshold at which this happens is low enough to be reached by most applications in which the residuals come from a simulation.

	\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.62\textwidth]{figures/fig15_breakeven_residual_cost}
	\caption{Proportion of the $47$ nonzero-residual instances on which Hybrid-SR1 is faster than the GN approximation, as a function of the extra cost charged to every residual evaluation in model~\eqref{eq:residual_cost_model}. The dotted line marks the median cost of a residual evaluation in the test set and the dashed line the median break-even cost~\eqref{eq:breakeven_cost}.}
	\label{fig:breakeven}
	\end{figure}

	The analysis of this section is produced by the script \texttt{residual\_cost\_analysis.jl} of the \texttt{benchmark} folder, from the same result files as the profiles above.}

	\mpierre{Based on these results, we retain the Hybrid-SR1 variant as the best overall strategy for TRAULLS, and use it in the comparisons that follow.}
	
	\subsection{Comparison with IPOPT and Percival}\label{subsec:compare_solvers}
	
	\mpierre{We now compare TRAULLS (with the Hybrid-SR1 approximation) against the solvers IPOPT~\cite{wachterbiegler:2006} and Percival~\cite{arreckx-etal:2016}. The latter is also an augmented Lagrangian algorithm and has a native Julia implementation. Its inner minimization uses a Julia version of the TRON~\cite{linmore:1999a} algorithm -- part of the \texttt{JSOSolvers.jl} package~\cite{migot-etal:2026}. To benefit from the benchmarking facilities provided by the JuliaSmoothOptimizers (JSO)\footnote{\url{https://jso.dev}} ecosystem, we used the wrapper \texttt{NLPModelsIpopt.jl}~\cite{nlp-models-ipopt-julia}. This allows us to run both reference solvers on the same test set as TRAULLS -- all models being encoded in \texttt{NLSProblems.jl}~\cite{nls-problems-julia} -- and to export the benchmark metrics using \texttt{SolverBenchmark.jl}~\cite{solver-benchmark-julia}. All solvers shared the experimental setup and the tolerances described at the beginning of this section, and were additionally given a maximum CPU time of 10 minutes per instance, a limit consistent with the dimensions of the problems considered.}
	
	\mpierre{To make the comparison with IPOPT relevant, we set the parameters \texttt{nlp\_scaling\_method} = \texttt{none}, \texttt{dual\_inf\_tol} = \texttt{Inf}, \texttt{constr\_viol\_tol} = \texttt{Inf}, \texttt{compl\_inf\_tol} = \texttt{Inf}, \texttt{acceptable\_iter} = \texttt{0} and the maximum number of iterations to 1000. We also set \texttt{hessian\_approximation} = \texttt{``limited-memory''}, so that IPOPT builds its Hessian model with an L-BFGS update of memory 6 instead of exact second derivatives. This choice makes the comparison meaningful with respect to the derivative information used. For Percival, since its outer iteration is the same as that of Algorithm~\ref{algo:traulls}, we set its tolerances and parameters to the same values as ours. We note that Percival, through its TRON inner solver, uses exact Hessian-vector products, and is therefore the only solver of the three that accesses second derivatives.}
	
	\mpierre{For these tests, we compare the three solvers with respect to the computation time, the number of residual evaluations and the number of gradient evaluations. After evaluating criterion~\eqref{eq:discarding_heuristic}, we removed from the profiles the following nine instances: HS61, HS77, HS264, the instances with $n=100$ of problems 5.1, 5.4, 5.13 and 5.14 from~\cite{lucksanvlcek:1999}, and the instances with $n=500$ and $n=1000$ of problem 5.16 from the same collection. The profiles of Figure~\ref{fig:perf_solvers} are thus built on the remaining 73 instances. On these, TRAULLS and IPOPT converge on every instance, while Percival reaches the maximum CPU time on the two instances of problem 5.11 from~\cite{lucksanvlcek:1999} with $n=500$ and $n=1000$. As a side note, we also ran IPOPT with exact second derivatives, and this configuration failed on two instances, reaching the maximum number of iterations on HS27 and terminating with a restoration-phase failure on HS70.}
	...
	\mpierre{In terms of computation time, TRAULLS is the fastest solver on more than 50\% of the instances. This advantage is, however, strongly dependent on the problem size. For instance, restricting the comparison to IPOPT, TRAULLS is faster on 40 of the 41 instances with $n \le 100$, but on only 8 of the 26 instances with $n \ge 500$, where our computation time is on average -- in the geometric mean sense -- $1.79$ times that of IPOPT. The margin observed on the smallest instances should be read with care, as both codes solve them in a few tens of milliseconds at most, with median times of $0.15$ and $1.5$ milliseconds respectively, so that the ratio there mostly reflects a lower fixed cost per solve rather than a difference in the optimization work performed. The large instances are where the gap is algorithmically meaningful, and there the cause is the dense storage and manipulation of the structured second-order correction, whose cost grows quadratically with the number of variables while the rest of the method only involves matrix-vector products. The example of problem 5.12 from~\cite{lucksanvlcek:1999} with $n=1000$ illustrates that point: Hybrid-SR1 requires 142 inner iterations against 354 for the GN approximation, yet takes $2.21$ seconds against $1.71$. This is the limitation we discuss in the conclusion, and it is the reason a limited-memory form of the structured update is the natural next step for large-scale problems.}
	
	\mpierre{Regarding the evaluation metrics, IPOPT dominates both profiles -- with a larger gap on gradients than on residuals -- and this dominance decomposes into two distinct effects. TRAULLS performs on average $1.91$ times as many residual evaluations as IPOPT, which reflects a genuine difference in the number of steps needed. On top of that, the two methods do not evaluate gradients at the same rate, since our trust-region scheme performs one gradient evaluation per inner iteration, whereas the line search in IPOPT reuses a single gradient across the trial points of a backtracking sequence. Part of the gap is thus structural, inherent to the globalization strategy rather than to the quality of our Hessian model.}
	
	\mpierre{These results should be read with the role of each solver in mind. IPOPT is a mature interior-point implementation, and it is included here as an external reference point rather than as the target of our contribution. The comparison that isolates what this work proposes is the one with Percival, which follows the same augmented Lagrangian framework, is run with the same tolerances and parameters, and differs from TRAULLS essentially in that it treats the problem as a generic nonlinear program. There, the advantage of TRAULLS is unambiguous and holds on all three metrics. \mfabian{In geometric mean over the $73$ retained instances, Percival takes $3.67$ times as long as TRAULLS, performs $2.94$ times as many residual evaluations and $1.47$ times as many gradient evaluations; TRAULLS is ahead on $60$, $66$ and $58$ of those instances respectively. Robustness follows the same pattern, TRAULLS being the only solver of the three to reach a first-order critical point on all $82$ instances of the test set.} Also, unlike the comparison with IPOPT, this advantage does not rest on the smallest instances; it is moreover obtained even though TRAULLS only approximates second-order information, whereas Percival evaluates exact Hessian-vector products. Within a common algorithmic framework, exploiting the least-squares structure through the Gauss--Newton term and its structured correction therefore leads to a clear improvement, which is the claim this paper makes. That TRAULLS is at the same time faster than IPOPT on most of the test set, while requiring more function evaluations, situates our implementation as a competitive solver whose remaining bottleneck -- the dense second-order correction on large instances -- is identified and addressable.}

	\mfabian{\subsection{Reproducibility}\label{subsec:reproducibility}

	Every number reported in this section can be regenerated from the repository. The \texttt{benchmark} folder contains a Julia environment, described by its \texttt{Project.toml} and \texttt{Manifest.toml} files, which pins the exact version of every package involved, TRAULLS included. The scripts \texttt{run\_traulls\_timed.jl} and \texttt{run\_percival\_ipopt\_timed.jl} run the solvers under the protocol described at the beginning of this section and write one result file per solver, recording for each instance the objective value reached, the elapsed time, the evaluation counters, the iteration counts and the termination status. Those files are the ones shipped in \texttt{results\_timed}, so that the profiles and the tables can be rebuilt without running the solvers again. The notebook \texttt{profiles.ipynb} produces the performance profiles and \texttt{residual\_cost\_analysis.jl} the break-even analysis of Section~\ref{subsec:compare_hessians}. The models that are not already available in \texttt{NLSProblems.jl}~\cite{nls-problems-julia} are given in the \texttt{traulls\_models} folder.}

	\section*{Conclusion}
	In this paper, we have presented an algorithm able to solve NLS problems subject to constraints of general form. Our method fully exploits the structure of the problem through the AL reformulation and a structured Hessian approximation. \mfabian{Three elements support it. The structured secant equation~\eqref{eq:structured_secant_eq} approximates, within a single update, the curvature carried by the residuals and the one carried by the nonlinear constraints through the shifted multipliers, and yields an approximation that also satisfies the standard secant equation. Proposition~\ref{prop:projected_path_polyhedron} establishes the properties of the projected gradient path over a general polyhedron on which the inner minimization rests, and which apply to any trust region method computing a Cauchy point over such a set. Together they give a solver covering a class of problems, namely least-squares objectives under nonlinear equalities combined with bounds and linear equalities kept explicit, for which no dedicated method was available.} \mpierre{The resulting implementation is faster and more robust than a traditional AL solver, and remains competitive in computation time with a state-of-the-art solver for nonlinear programming. Despite this promising performance, the main limitation for} large-scale applications lies in the dense storage of the structured second-order correction of the Hessian, in addition to the subsequent computations that become prohibitive as the number of variables grows. A limited-memory variant of the structured update, following~\cite{byrd-etal:1994}, would be a natural feature to implement. \mfabian{A second open question is local. In the unconstrained case, structured updates are known to restore a fast asymptotic rate on problems with nonzero residuals at the solution, and the iteration counts reported in Section~\ref{sec:numerical_experiments} are consistent with a similar effect here. Establishing it would require a local analysis of the inner minimization under the structured secant equation~\eqref{eq:structured_secant_eq}, which we have not attempted and which is not covered by the global results of Section~\ref{sec:convergence_analysis}.} To tackle problems with a large number of residuals, another avenue of research could be to combine the AL framework with an adaptive sampling strategy applied to the residuals, lowering the dimensions of the inner minimization subproblems. This would extend to the constrained case techniques currently investigated for unconstrained problems~\cite{bellavia-etal:2026}.
	...
	We describe the procedure used to compute the first local minimizer of the quadratic $q(s) = \frac{1}{2} \inner{s}{Hs} + \inner{g}{s}$ along the projected gradient path $s(t) = \proj{\Omega}{x-tg}-x$ with $t \ge 0$. This is an adaptation of Algorithm 17.3.1 from~\cite{conn-etal:2000} to the case where linear equalities are present. 
	For each $t \ge 0$, the projected step $s(t)$ satisfies the constraints
	\[As(t) = 0, \qquad s^{(l)} \le s(t) \le s^{(u)},\]
	where matrix $A$ is $m\times n$ with $m < n$ and vectors $s^{(l)}$, $s^{(u)}$ are iteration-dependent bounds having finite components.
	The breakpoints
	\begin{equation*}
		0=t_0 < t_1 < \ldots < t_p,
	\end{equation*} 
	correspond to the successive scalars at which at least one component of $s(t)$ satisfies a bound with equality. Assuming $A$ has full row rank, there are $n-m$ degrees of freedom remaining and thus at most $n-m$ breakpoints before the projected gradient path is constant. We recall the recursive expression
	\[s(t) = -(t-t_i)z_i + s(t_i),\]
	...
	\begin{align*}
		&\psi_i' > 0 \text{ or } \\
		&\left(\psi'_i=0 \text{ and } \psi_i'' > 0\right),
	\end{align*}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\If{$\psi_i' > 0$ or $\left(\psi'_i=0 \text{ and } \psi_i'' > 0\right)$}
