...
function jc(x)
    ...
    for k=1:p
        res[k,k] = -(x[k]+1) * exp(x[k]-x[k+1])
        res[k,k+1] = 9x[k+1]^2 + cos(x[k+1]-x[k+2])*sin(x[k+1]+x[k+2]) + sin(x[k+1]-x[k+2])*cos(x[k+1]+x[k+2]) + 4 + x[k]*exp(x[k]-x[k+1])
        res[k,k+2] = 2 - cos(x[k+1]-x[k+2])*sin(x[k+1]+x[k+2]) + sin(x[k+1]-x[k+2])*cos(x[k+1]+x[k+2])
    end
...
@testset "Sparse model with nonlinear equality constraints" begin
    ...
    # Test jacobian evaluation functions return sparse arrays
    ...
    x .+= 1.0
    Traulls.jac_residuals!(model, J, x)
    Traulls.jac_nlconstraints!(model, C, x)
    @test typeof(J) <: AbstractSparseMatrix  && typeof(C) <: AbstractSparseMatrix
    @test Matrix(J) ≈ jr(x) && Matrix(C) ≈ jc(x)
    (irows, icols, _) = findnz(J)
    @test size(irows, 1) == size(jr_nzrows, 1)
    @test size(icols, 1) == size(jr_nzcols, 1)
    (irows, icols, _) = findnz(C)
...
@testset "Hessian approximation with sparse jacobians" begin

    J = Traulls.jac_residuals(model, x)
    C = Traulls.jac_nlconstraints(model, x)
    H = Traulls.GN(J, C, 1.0)

    @test typeof(H.J) <: AbstractSparseMatrix && typeof(H.C) <: AbstractSparseMatrix
    J.nzval .+= 1.0
    C.nzval .+= 1.0
    Traulls.update_hessian!(H, J, C)
    @test typeof(H.J) <: AbstractSparseMatrix && typeof(H.C) <: AbstractSparseMatrix
    @test H.J.nzval ≈ J.nzval && H.C.nzval ≈ C.nzval
end
